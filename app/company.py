...
def company():
    if current_user:
        ...
        elif request.method == 'POST' and request.form and (current_user.userTypeId == 1 or
                                                            (current_user.userTypeId == 4 and datetime.strptime(
                                                                current_user.valid_until,
...
@login_required
def company_info(id):
    if current_user.userTypeId == 1 or (current_user.userTypeId == 4 and datetime.strptime(
            current_user.valid_until, "%Y-%m-%d").date() > datetime.date.today()):
        session = Session()
        # try:
        if request.method == 'GET':
...
def signup_worker(id_company=None):
    if request.method == 'GET' and (current_user.userTypeId == 1 or
                                    (current_user.userTypeId == 4 and datetime.strptime(
                                        current_user.valid_until,
                                        "%Y-%m-%d").date() > datetime.date.today())):
...
def worker_delete(id):
    if current_user and request.method == "DELETE" and (current_user.userTypeId == 1 or
                                                        (current_user.userTypeId == 4 and datetime.strptime(
                                                            current_user.valid_until,
                                                            "%Y-%m-%d").date() > datetime.date.today())):
        session = Session()
